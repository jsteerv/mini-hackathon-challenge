...
async def retrieve_relevant_documents_tool(supabase: Client, embedding_client: AsyncOpenAI, user_query: str,
                                          file_id: Optional[str] = None, tag: Optional[str] = None) -> str:
    """
...
async def list_documents_tool(supabase: Client) -> List[str]:
    ...
    try:
        ...
        result = supabase.from_('document_metadata') \
            .select('id, title, schema, url, tags') \
            .execute()
