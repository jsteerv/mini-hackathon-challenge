...
from text_processor import chunk_text, create_embeddings, is_tabular_file, extract_schema_from_csv, extract_rows_from_csv, extract_tags_from_markdown
...
def insert_document_chunks(chunks: List[str], embeddings: List[List[float]], file_id: str,
                        file_url: str, file_title: str, mime_type: str, file_contents: bytes | None = None, tags: Optional[List[str]] = None) -> None:
    """
    ...
    Args:
        ...
        tags: Optional list of tags extracted from markdown files
    """
    try:
...
def insert_or_update_document_metadata(file_id: str, file_title: str, file_url: str, schema: Optional[List[str]] = None, tags: Optional[List[str]] = None) -> None:
    """
...
def process_file_for_rag(file_content: bytes, text: str, file_id: str, file_url: str, 
    ...
    try:
        ...
        # Extract tags if this is a markdown file
        tags = []
        if mime_type and ('markdown' in mime_type.lower() or '.md' in file_title.lower()):
            tags = extract_tags_from_markdown(text)
            if tags:
                print(f"Extracted {len(tags)} tags from '{file_title}': {', '.join(tags)}")

        # First, insert or update document metadata (needed for foreign key constraint)
        insert_or_update_document_metadata(file_id, file_title, file_url, schema, tags)
        ...
        if mime_type.startswith("image"):
            insert_document_chunks(chunks, embeddings, file_id, file_url, file_title, mime_type, file_content, tags)
            return True

        # Insert the chunks with their embeddings
        insert_document_chunks(chunks, embeddings, file_id, file_url, file_title, mime_type, None, tags)
