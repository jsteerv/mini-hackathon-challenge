...
class GoogleDriveWatcher:
    ...
    def get_folder_contents(self, folder_id: str, time_str: str) -> List[Dict[str, Any]]:
        ...
        # Recursively get contents of each subfolder (skip excluded folders)
        for subfolder in subfolders:
            # Skip excluded folders
            if subfolder.get('name') in excluded_folders:
                print(f"Skipping excluded folder: {subfolder.get('name')}")
                continue
