...
async def telephony_entrypoint(ctx: agents.JobContext):
    ...
    print(f"Telephony agent started: {call_type}")
    
    # Monitor telephony-specific events
    @ctx.room.on("participant_disconnected")
    async def on_participant_disconnected(participant):
        print(f"Call ended: {participant.identity}")
    ...
    def on_dtmf(dtmf_event):
        ...
        print(f"DTMF received: {dtmf_event.digit}")
        
        # For IVR, process the digit
        if isinstance(agent, IVRAgent):
            asyncio.create_task(
                agent.route_call(None, str(dtmf_event.digit))
            )
