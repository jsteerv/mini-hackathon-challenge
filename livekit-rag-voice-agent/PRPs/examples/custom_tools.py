...
async def entrypoint(ctx: agents.JobContext):
    ...
    print(f"Tools assistant started with {len(assistant.tools)} tools")
    
    # Monitor tool usage
    @session.on("tool_call_started")
    def on_tool_started(ev):
        print(f"Tool started: {ev.tool_name}")
    
    @session.on("tool_call_completed")
    def on_tool_completed(ev):
        print(f"Tool completed: {ev.tool_name} -> {ev.result[:100]}")
