...
async def openai_realtime_entrypoint(ctx: agents.JobContext):
    ...
    print(f"Realtime agent started in room: {ctx.room.name}")
    
    # Monitor realtime-specific events
    @session.on("realtime_response_started")
    def on_response_started():
        print("Realtime response started")
    
    @session.on("realtime_response_done")
...
async def gemini_vision_entrypoint(ctx: agents.JobContext):
    ...
    print(f"Vision agent started in room: {ctx.room.name}")
    
    # Monitor video-specific events
    @ctx.room.on("track_subscribed")
    def on_track_subscribed(track):
        if track.kind == "video":
            print(f"Video track subscribed: {track.sid}")
    
    @session.on("vision_frame_processed")
    def on_frame_processed(frame_info):
        print(f"Processed video frame: {frame_info}")
...
async def combined_entrypoint(ctx: agents.JobContext):
    ...
    print(f"Multimodal agent started with video={'enabled' if has_video else 'disabled'}")
...
if __name__ == "__main__":
    ...
    agents.cli.run_app(
        agents.WorkerOptions(
            ...
            worker_name="multimodal-assistant",
        )
    )