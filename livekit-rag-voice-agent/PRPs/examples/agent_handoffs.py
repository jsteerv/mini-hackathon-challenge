...
async def entrypoint(ctx: agents.JobContext):
    ...
    session = AgentSession(
        ...
        llm=openai.LLM(model="gpt-4o"),  # More capable model for complex routing
    ...
    print(f"Multi-agent system started in room: {ctx.room.name}")
    
    # Track agent transitions
    @session.on("agent_changed")
    def on_agent_changed(ev):
        print(f"Agent changed: {ev.old_agent} -> {ev.new_agent}")
    
    # Monitor handoff events
    @session.on("handoff_started")
    def on_handoff_started(ev):
        print(f"Handoff initiated: {ev.reason}")
    
    @session.on("handoff_completed")
    def on_handoff_completed(ev):
...
if __name__ == "__main__":
    agents.cli.run_app(
        agents.WorkerOptions(
            ...
            worker_name="multi-agent-system",
        )
    )