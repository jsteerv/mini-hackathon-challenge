...
async def mcp_entrypoint(ctx: agents.JobContext):
    ...
    print(f"MCP-integrated agent started with {len(mcp_servers)} servers")
    
    # Monitor MCP events
    @session.on("mcp_tool_called")
    def on_mcp_tool(ev):
        print(f"MCP tool called: {ev.tool_name} on {ev.server}")
    
    @session.on("mcp_error")
    def on_mcp_error(ev):
        print(f"MCP error: {ev.error} from {ev.server}")
...
if __name__ == "__main__":
    agents.cli.run_app(
        agents.WorkerOptions(
            ...
            worker_name="mcp-integrated-agent",
        )
    )