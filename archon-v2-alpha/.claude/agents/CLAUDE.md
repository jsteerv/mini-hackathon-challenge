...
**CRITICAL: This project uses Archon MCP server for task and project management. All agents MUST follow these protocols:**

### Initial Setup for All Agents

**MANDATORY: Every agent must run this command first:**
```
mcp__serena__initial_instructions()
```

This provides essential project context and structure understanding.

### Proper MCP Tool Usage

**CORRECT Syntax (Direct MCP invocation):**
```python
# Project Management
mcp__archon__manage_project(action="create", title="My Project")
mcp__archon__manage_project(action="list")
mcp__archon__manage_project(action="get", project_id="uuid-here")

# Task Management
mcp__archon__manage_task(action="create", project_id="uuid", title="Task Name")
mcp__archon__manage_task(action="list", filter_by="status", filter_value="todo")
mcp__archon__manage_task(action="update", task_id="uuid", update_fields={"status": "doing"})

# Document Management
mcp__archon__manage_document(action="add", project_id="uuid", document_type="prp", title="Doc Title", content={})

# Knowledge Base
mcp__archon__perform_rag_query(query="search term", match_count=5)
mcp__archon__search_code_examples(query="implementation pattern", match_count=3)
```

**INCORRECT (Never do this):**
```bash
# ❌ WRONG - This launches a new Claude instance
claude /mcp__archon__manage_project create "My Project"
bash claude mcp__archon__manage_task
```

### Context Passing Protocol

When the agent-organizer dispatches tasks to specialized agents, it MUST provide:
- `project_id`: The Archon project UUID
- `task_id`: Specific task UUID (if applicable)
- `feature_name`: Feature context for organization
- Any other relevant IDs or context

Sub-agents MUST receive and use this context rather than creating new projects/tasks.
