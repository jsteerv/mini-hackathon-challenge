...
**CRITICAL: When Archon MCP server is available, ALL project and task management MUST go through Archon.**

### Initial Archon Check

Before any project analysis, you MUST:
1. Check if Archon MCP tools are available by attempting: `mcp__archon__manage_project(action="list")`
2. If successful, check for existing projects related to the user's request
3. Run `mcp__serena__initial_instructions()` to understand project context

### Context Passing Requirements

When dispatching tasks to specialized agents, you MUST provide:
```yaml
Essential Context:
  project_id: "uuid-from-archon"  # MANDATORY if using Archon
  task_id: "specific-task-uuid"    # If working on specific task
  feature_name: "feature-context"  # For organization
  
Instructions for Sub-Agents:
  - "First run: mcp__serena__initial_instructions()"
  - "Use provided project_id for all Archon operations"
  - "Do NOT create new projects - use existing project_id"
  - "Update task status when starting and completing work"
```

### Archon Workflow Integration

1. **For New Features/Projects:**
   - Create project via `mcp__archon__manage_project`
   - Pass project_id to ALL sub-agents
   - Ensure sub-agents create tasks under this project

2. **For Existing Projects:**
   - Query existing tasks first
   - Pass both project_id and relevant task_ids
   - Sub-agents must update task status, not create duplicates
...
- **Archon Context:** Project ID, task IDs, and specific instructions for MCP tool usage
...
**MANDATORY Archon Context for All Agents:**
```yaml
Archon Integration:
  project_id: "[uuid-from-archon-project]"
  Initial Instructions: "Run mcp__serena__initial_instructions() first"
  MCP Tool Usage: "Use mcp__archon__* tools directly, not bash commands"
  Task Updates: "Update task status when starting and completing work"
```
